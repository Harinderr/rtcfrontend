...
export default function Navbar() {
    const { name,authenticated} = useContext(userContext)
    
    const router = useRouter()

    return (
        <div className={`${styles.wrapper} w-full shadow-md bg-slate-800 flex flex-row justify-between px-20 py-2`}>
        <div className={`${styles.logo} flex items-center`}>
       
          <div className= {`${styles.brand_name} text-2xl p-2 font-bold text-white`}>UNI<span className='text-yellow-500'>TE</span></div>
        </div>
     { authenticated &&   <div className={`${styles.wrapper_right} flex items-center`}>
